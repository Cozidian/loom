...
// raceArena is a presentation projection of durable OTP runtime events. It
// never decides who is ahead or who won; only race_winner_selected may assign
// WinnerID. Completion therefore means submitted for judgment, not leading.
...
func (m *model) applyRaceEvent(eventType string, data map[string]any, sessionID string, root, hydrating bool) bool {
	...
	case "provider_auction_awarded":
		arena := m.raceByAuction(asString(data["auction_id"]))
		if arena == nil || asString(data["purpose"]) != "provider_race" {
			return false
		}
		arena.Phase = "awarded"
	...
	case "race_candidate_completed":
		...
		if arena.allCandidatesSubmitted() {
			arena.Phase = "judging"
...
func (m model) renderCompactRace(arena raceArena) string {
	...
	header := joinEdges("RACE ARENA", strings.ToUpper(arena.Phase), width)
	lines := []string{
		styleMint.Bold(true).Render(header),
		styleFaint.Render(raceStageRail(arena.Phase)),
	}
...
func (m model) renderRaceLane(arena raceArena, candidate raceCandidate, width int, selected bool) string {
	...
	case candidate.Status == "failed" || candidate.Status == "stopped":
...
func (m model) renderRaceTab() string {
	...
	if arena == nil {
		return lipgloss.NewStyle().Padding(2, 2).Foreground(colDim).
			Render("No provider race yet.\n\nUse /race GOAL to ask several providers for independent candidates.")
	}

	width := max(28, m.width-4)
	var b strings.Builder
	fmt.Fprintf(&b, "%s\n", styleMint.Bold(true).Render("RACE ARENA"))
	fmt.Fprintf(&b, "%s\n", bodyStyle.Render("Parallel proposals, then independent judgment. Finishing first does not win."))
	fmt.Fprintf(&b, "%s\n\n", raceStageRail(arena.Phase))
	...
	if len(arena.Bids) > 0 {
		fmt.Fprintf(&b, "\n%s\n", styleFaint.Render("PROVIDER BIDS · ROUTING FIT, NOT RACE POSITION"))
	...
	fmt.Fprintf(&b, "\n%s\n", styleFaint.Render("RACE STATE"))
	fmt.Fprintf(&b, "%s\n", bodyStyle.Render(arena.Status))
	if arena.WinnerEndpoint != "" {
		fmt.Fprintf(&b, "%s\n", styleMint.Render("selected by evaluator  "+arena.WinnerEndpoint))
	}

# Earlier roadmap and implementation history

Archived on 2026-09-10 after the product-vision interview. This preserves the
previous work order, rationale and recorded implementation evidence; its
checkboxes are historical claims, not freshly verified status or instructions
to implement the remaining items. Some architectural language and priorities
have been superseded by the broader code-and-document vision.

Read [VISION.md](../VISION.md) for evolving intent and [ROADMAP.md](../ROADMAP.md)
for current experiments. Neither is a specification. Revisit earlier ideas when
they help, but do not treat this archive as a parallel backlog.
...
- [x] Replace fixed helpers with task-based same-model subagents

    - Remove the two-cheap-helper launcher and four-task tool ceiling; allow one
      endpoint/model to serve many independent workers.
    - Let goal worker capacity drive coordinator graph parallelism. Queue excess
      graph work without failed attempts; expose worker/model capacity at startup.
    - Preserve scoped editors, dependency handoffs, budgets and recursion policy.
      Tie turn-owned graph cancellation to its owner and reclaim abandoned queues.
    - Show capacity waits and task starts in ION. See [task teams](task-teams.md).
      Hardware/quota discovery and cross-machine worker execution remain future work.

- [x] Add automatic specialization with visible ownership

    Historical first slice; its fixed launcher is superseded by task-based teams above.
...
- [x] Make ordinary coding requests executable without a mandatory provider team
    ...
    independent review. See [evidence and limits](coding-usability.md).

...
[Overview](../README.md) · [Vision](../VISION.md) · [Current roadmap](../ROADMAP.md)
...
In `--team-mode auto`, the owner can organize useful work into a task graph,
including several subagents using **the same provider and model**. Model routing
is independent: `--model-strategy manual --team-mode auto` pins the selected
model without disabling teams. No cheap second endpoint is required, and there
is no fixed two-helper or four-task ceiling. Small or tightly coupled work can
stay solo; agents are not launched just to fill slots.
...
`--max-workers` limits simultaneous subagent allocations per goal; extra task-graph
nodes wait. `--model-concurrency` configures slots in each project model pool,
including the expensive-model pool. Both default to 4 and accept positive
integers without a fixed upper cap. Save them through `init` or the optional
`max_workers` / `model_concurrency` config fields; run flags are temporary.
These are operator-set capacity limits, not detected cloud quotas or GPU limits.
Restart the harness to change project pool sizes.

Parallel editors must declare disjoint path authority or ordered dependency
handoffs. The owner integrates and verifies; permissions, evidence requirements,
budgets and recursion safeguards still apply. ION shows task starts and capacity
waits. Execution remains local; remote providers are not remote worker processes.
See [task-based teams](task-teams.md) for lifecycle and scheduling boundaries.
...
See [ROADMAP.md](../ROADMAP.md) for the current experiments and proposed next work.
